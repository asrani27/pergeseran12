...
@section('title', 'Edit Pengajuan Pergeseran')
...
<div class="transition-all duration-300">
    ...
    <div class="mb-6">
        <div class="bg-gradient-to-r from-yellow-500 to-orange-500 rounded-xl p-6 text-white shadow-lg">
            <div class="text-center">
                <h1 class="text-3xl font-bold mb-2">Edit Pengajuan</h1>
                <p class="text-yellow-100">Silahkan edit form di bawah ini untuk memperbarui pengajuan</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <form role="form" method="post" action="{{ route('skpd.pengajuan.update', $pengajuan->id) }}">
            @csrf
            @method('PUT')
            <div class="p-6">
                <!-- Form Grid -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="tanggal" name="tanggal" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            value="{{ old('tanggal', \Carbon\Carbon::parse($pengajuan->tanggal)->format('Y-m-d')) }}">
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <select id="tipe_pengajuan" name="tipe_pengajuan" required
                        ...
                        <option value="1" {{ old('tipe_pengajuan', $pengajuan->tipe_pengajuan) == 1 ? 'selected' : '' }}>Antar Objek</option>
                        <option value="2" {{ old('tipe_pengajuan', $pengajuan->tipe_pengajuan) == 2 ? 'selected' : '' }}>Antar Rincian Objek</option>
                        <option value="3" {{ old('tipe_pengajuan', $pengajuan->tipe_pengajuan) == 3 ? 'selected' : '' }}>Antar Sub Rincian Objek</option>
                        <option value="4" {{ old('tipe_pengajuan', $pengajuan->tipe_pengajuan) == 4 ? 'selected' : '' }}>Perubahan Uraian</option>
                    </select>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea id="pengantar" name="pengantar" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="Masukkan kalimat pengantar">{{ old('pengantar', $pengajuan->pengantar) }}</textarea>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="mb-2">
                        ...
                        <a href="{{ asset('storage/' . $pengajuan->lampiran) }}" target="_blank" class="text-blue-600 hover:underline text-sm">
                            Lihat Lampiran
                        </a>
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <select id="program" name="program" required
                            ...
                            <option value="{{$item->id}}" 
                                ...
                                {{ old('program', $pengajuan->program_id) == $item->id ? 'selected' : '' }}>
                                {{$item->kode}} - {{$item->nama}}
                            </option>
                            @endforeach
                            @endif
...
<script>
    // Store current pengajuan data for pre-selecting
    const currentPengajuan = {
        program_id: {{ $pengajuan->program_id }},
        kegiatan_id: {{ $pengajuan->kegiatan_id }},
        subkegiatan_id: {{ $pengajuan->subkegiatan_id }},
        program: {
            kode_skpd: '{{ $pengajuan->program->kode_skpd }}',
            kode: '{{ $pengajuan->program->kode }}',
            tahun: '{{ $pengajuan->program->tahun }}'
        },
        kegiatan: {
            kode: '{{ $pengajuan->kegiatan->kode }}'
        }
    };
        ...
        if (programId) {
            axios.get(`/kegiatan/${programId}`, {
                ...
                .then(function(response) {
                    ...
                    // Pre-select current kegiatan if it matches the selected program
                    if (programId == currentPengajuan.program_id) {
                        kegiatanSelect.value = currentPengajuan.kegiatan_id;
                        loadSubkegiatan();
                    }
                })
    ...
    // Load subkegiatan based on selected kegiatan
    function loadSubkegiatan() {
        ...
        const selectedKegiatanOption = kegiatanSelect.options[kegiatanSelect.selectedIndex];
        const kodeKegiatan = selectedKegiatanOption ? selectedKegiatanOption.getAttribute('data-kode-kegiatan') : '';
        ...
        if (kegiatanId) {
            axios.get(`/subkegiatan/${kegiatanId}`, {
                ...
                .then(function(response) {
                    ...
                    // Pre-select current subkegiatan if it matches the selected kegiatan
                    if (kegiatanId == currentPengajuan.kegiatan_id) {
                        subkegiatanSelect.value = currentPengajuan.subkegiatan_id;
                    }
                })
                .catch(function(error) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Load kegiatan and subkegiatan for the currently selected program
        loadKegiatan();
    });
</script>
@endpush
